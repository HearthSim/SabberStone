...
namespace SabberStone.Model
{
    ...
    public class Zone<T> : IZone, IEnumerable<T> where T : IPlayable
    {
        ...
        public Zone(Game game, Controller controller, Zone type)
        {
            ...
            Type = type;
            Log.Debug($"Created Zone {type} in Game with Controller {controller.Name}");
        }
        ...
        public IPlayable Add(IPlayable entity, int zonePosition = -1)
        {
            ...
            Log.Debug($"Entity '{entity} ({entity.Card.Type})' has been added to zone '{Type}' in position '{entity[GameTag.ZONE_POSITION]}'.");
