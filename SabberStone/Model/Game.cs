...
namespace SabberStone.Model
{
    ...
    public partial class Game : Entity
    {
        ...
        public StringBuilder PlayTaskLog { get; } = new StringBuilder();
        ...
        {
            Log.Info($"Starting new game now!");
            PlayTaskLog.AppendLine($"Starting new game now!");
            ...
            Log.Info($"Starting Player is {CurrentPlayer.Name}.");
            PlayTaskLog.AppendLine($"Starting Player is {CurrentPlayer.Name}.");
        ...
        public void BeginTurnPlayer()
        {
            ...
            Log.Info($"[T:{Turn}/R:{(int) Turn / 2}] with CurrentPlayer {CurrentPlayer.Name} " +
                     $"[HP:{CurrentPlayer.Hero.Health}/M:{CurrentPlayer.RemainingMana}]");
            PlayTaskLog.AppendLine($"[T:{Turn}/R:{(int)Turn / 2}] with CurrentPlayer {CurrentPlayer.Name} " +
                     $"[HP:{CurrentPlayer.Hero.Health}/M:{CurrentPlayer.RemainingMana}]");
        ...
        {
            Log.Info($"End turn proccessed by player {CurrentPlayer}");
            PlayTaskLog.AppendLine($"End turn proccessed by player {CurrentPlayer}");
        ...
        public void EndTurn()
        {
            ...
            Log.Info($"CurentPlayer {CurrentPlayer.Name}.");
            PlayTaskLog.AppendLine($"CurentPlayer {CurrentPlayer.Name}.");
        ...
        public void GraveYard()
        {
            ...
            {
                Log.Info($"{p} is Dead! Graveyard say 'Hello'!");
                PlayTaskLog.AppendLine($"{p} is Dead! Graveyard say 'Hello'!");
                p.Zone.Remove(p);
                if (p.HasDeathrattle)
                {
                    p.ApplyEnchantments(EnchantmentActivation.DEATHRATTLE, HearthDb.Enums.Zone.GRAVEYARD);
                }
        ...
        public void DeathProcessingAndAuraUpdate()
        {
            ...
            while (TaskQueue.Count > 0)
            {
                if (TaskQueue.Process() != TaskState.COMPLETE)
                    Log.Error($"Something really bad happend during proccessing, please analyze!");
        ...
        public void GameOver()
        {
            ...
            _players.ToList().ForEach(p =>
            {
                Log.Info($"{p.Name} has {p.PlayState} the Game!");
                PlayTaskLog.AppendLine($"{p.Name} has {p.PlayState} the Game!");
            });
        }
        ...
        public void Process(PlayerTask gameTask)
        {
            ...
            PlayTaskLog.AppendLine(gameTask.FullPrint());
            ...
            if (Splitting)
            {
                ...
                Log.Info($"found {finalSplits.Count} final splits!");
                finalSplits.GroupBy(p => p.SameState)
                    .Select(i => new {Word = i.Key, Count = i.Count()})
                    .ToList().ForEach(p => Log.Info($"{p.Word} - {p.Count}"));
            }
        }
