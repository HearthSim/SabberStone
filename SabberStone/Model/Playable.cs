...
namespace SabberStone.Model
{
    ...
    public abstract partial class Playable<T> : Targeting, IPlayable where T : Entity
    {
        ...
        {
            ToBeDestroyed = true;
            Log.Info($"{this} just got set to be destroyed.");
            Game.PlayTaskLog.AppendLine($"{this} just got set to be destroyed.");
            return this;
        }
        ...
        public virtual bool IsPlayable
        {
            get
            {
                ...
                { 
                    Log.Info($"{this} isn't playable, because player not on turn.");
                    return false;
                }
                ...
                {
                    Log.Info($"{this} isn't playable, because card not in hand.");
                    return false;
                }
                ...
                {
                    Log.Info($"{this} isn't playable, because not enough mana to pay cost.");
                    return false;
                }
                ...
                {
                    Log.Info($"{this} isn't playable, because need valid target and we don't have one.");
                    return false;
                }
                ...
                foreach (var item in RefCard.Requirements)
                {
                    ...
                    Log.Debug($"{this} check PlayReq {req} ... !");
                    ...
                    switch (req)
                    {
                        case PlayReq.REQ_NUM_MINION_SLOTS:
                            ...
                            {
                                Log.Info($"Board is full can't summon new minion.");
                                return false;
                            }
                            break;
                        case PlayReq.REQ_ENTIRE_ENTOURAGE_NOT_IN_PLAY:
                            ...
                            if (containsAll)
                            {
                                Log.Info($"All ready all entourageing cards summoned.");
                                return false;
                            }
                            break;
                        case PlayReq.REQ_WEAPON_EQUIPPED:
                            ...
                            {
                                Log.Info($"Need a weapon to play this card.");
                                return false;
                            }
                            break;
                        case PlayReq.REQ_MINIMUM_ENEMY_MINIONS:
                            ...
                            {
                                Log.Info($"Need at least {param} enemy minions to play this card.");
                                return false;
                            }
                            break;
                        case PlayReq.REQ_STEADY_SHOT:
                            ...
                            {
                                Log.Info($"Need steady shoot to be used.");
                                return false;
                            }
                            break;
                        ...
                        default:
                            Log.Info($"PlayReq {req} not in switch needs to be added (Playable)!");
                            break;
                    }
                }
