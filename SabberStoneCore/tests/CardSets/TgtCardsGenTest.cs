...
namespace SabberStoneCoreTest.CardSets
{
	...
	public class DruidTgtTest
	{
		...
		public void Mulch_AT_044()
		{
			...
			Assert.Equal(ECardType.MINION, game.CurrentOpponent.Hand[4].Card.Type);
		}
		...
		public void DruidOfTheSaber_AT_042()
		{
			...
			Assert.Equal(true, ((Minion)game.CurrentPlayer.Board[1]).HasStealth);
		}
	...
	public class MageTgtTest
	{
		...
		public void Effigy_AT_002()
		{
			...
			Assert.Equal(1, game.CurrentOpponent.Board.Count);
			Assert.Equal(true, minion.ToBeDestroyed);
			Assert.Equal(minion.Cost, game.CurrentOpponent.Board[0].Cost);
	...
	public class PaladinTgtTest
	{
		...
		public void CompetitiveSpirit_AT_073()
		{
			...
			game.Process(EndTurnTask.Any(game.CurrentPlayer));
			game.Process(EndTurnTask.Any(game.CurrentPlayer));
			Assert.Equal(0, game.CurrentPlayer.Secrets.Count);
		...
		public void MurlocKnight_AT_076()
		{
			...
			game.Process(HeroPowerTask.Any(game.CurrentPlayer));
			Assert.Equal(3, game.CurrentPlayer.Board.Count);
			Assert.Equal(ERace.MURLOC, game.CurrentPlayer.Board[2].Card.Race);
		}
		...
		public void ArgentLance_AT_077()
		{
			...
			game.Process(PlayCardTask.Spell(game.CurrentPlayer, testCard));
			Assert.NotEqual(null, game.CurrentPlayer.Hero.Weapon);
	...
	public class PriestTgtTest
	{
		...
		public void Convert_AT_015()
		{
			...
			game.Process(PlayCardTask.SpellTarget(game.CurrentPlayer, testCard, minion));
			Assert.Equal(6, game.CurrentPlayer.Hand.Count);
			Assert.Equal(minion.Card.Id, game.CurrentPlayer.Hand[5].Card.Id);
		}
		...
		public void ConfessorPaletress_AT_018()
		{
			...
			Assert.Equal(2, game.CurrentPlayer.Board.Count);
			Assert.Equal(ERarity.LEGENDARY, game.CurrentPlayer.Board[1].Card.Rarity);
		}
		...
		public void WyrmrestAgent_AT_116()
		{
			...
			game.Process(PlayCardTask.Minion(game.CurrentPlayer, testCard));
			Assert.Equal(true, ((Minion)testCard).HasTaunt);
	...
	public class RogueTgtTest
	{
		...
		public void Burgle_AT_033()
		{
			...
			game.Process(PlayCardTask.Spell(game.CurrentPlayer, testCard));
			Assert.Equal(6, game.CurrentPlayer.Hand.Count);
			Assert.Equal(ECardClass.PRIEST, game.CurrentPlayer.Hand[5].Card.Class);
			Assert.Equal(ECardClass.PRIEST, game.CurrentPlayer.Hand[4].Card.Class);
		}
		...
		public void Buccaneer_AT_029()
		{
			...
			Assert.Equal(game.CurrentPlayer.Hero.Weapon.Card[EGameTag.ATK] + 1,
	...
	public class ShamanTgtTest
	{
		...
		public void ThunderBluffValiant_AT_049()
		{
			...
			game.Process(PlayCardTask.Minion(game.CurrentPlayer, testCard));
			game.Process(HeroPowerTask.Any(game.CurrentPlayer));
			Assert.Equal(game.CurrentPlayer.Board[1].Card[EGameTag.ATK] + 2,
		...
		public void TheMistcaller_AT_054()
		{
			...
			if (minionsHand.Any())
			{
				...
				var oAtk = minionsHand.Sum(p => ((Minion)p).Card[EGameTag.ATK]);
				Assert.Equal(oAtk + count, nAtk);
				var nHp = minionsHand.Sum(p => ((Minion)p).Health);
				var oHp = minionsHand.Sum(p => ((Minion)p).Card[EGameTag.HEALTH]);
			...
			if (minionsDeck.Any())
			{
				...
				var oAtk = minionsDeck.Sum(p => ((Minion)p).Card[EGameTag.ATK]);
				Assert.Equal(oAtk + count, nAtk);
				var nHp = minionsDeck.Sum(p => ((Minion)p).Health);
				var oHp = minionsDeck.Sum(p => ((Minion)p).Card[EGameTag.HEALTH]);
				Assert.Equal(oHp + count, nHp);
			}
		}
	...
	public class WarlockTgtTest
	{
		...
		public void Dreadsteed_AT_019()
		{
			...
			game.Process(EndTurnTask.Any(game.CurrentPlayer));
			game.Process(HeroPowerTask.Any(game.CurrentPlayer, testCard));
			Assert.Equal(true, ((Minion)testCard).IsDead);
			Assert.Equal(1, game.CurrentOpponent.Board.Count);
			Assert.Equal(testCard.Card.Id, game.CurrentOpponent.Board[0].Card.Id);
		}
	...
	public class WarriorTgtTest
	{
		...
		public void VarianWrynn_AT_072()
		{
			...
			game.Process(PlayCardTask.Minion(game.CurrentPlayer, testCard));
			Assert.Equal(count + 3, game.CurrentPlayer.Hand.Count + game.CurrentPlayer.Board.Count);
		}
	...
	public class NeutralTgtTest
	{
		...
		public void DragonhawkRider_AT_083()
		{
			...
			game.Process(PlayCardTask.Minion(game.CurrentPlayer, testCard));
			Assert.Equal(false, ((Minion)testCard).HasWindfury);
			game.Process(HeroPowerTask.Any(game.CurrentPlayer));
			Assert.Equal(true, ((Minion)testCard).HasWindfury);
			game.Process(EndTurnTask.Any(game.CurrentPlayer));
			Assert.Equal(false, ((Minion)testCard).HasWindfury);
		}
		...
		public void Saboteur_AT_086()
		{
			...
			game.Process(PlayCardTask.Minion(game.CurrentPlayer, testCard));
			game.Process(EndTurnTask.Any(game.CurrentPlayer));
			Assert.Equal(7, game.CurrentPlayer.Hero.Power.Cost);
			game.Process(EndTurnTask.Any(game.CurrentPlayer));
			game.Process(EndTurnTask.Any(game.CurrentPlayer));
			Assert.Equal(2, game.CurrentPlayer.Hero.Power.Cost);
		}
		...
		public void MogorsChampion_AT_088()
		{
			...
			Assert.Equal(true, game.CurrentOpponent.Hero.Health < 30 || !right1 || !right2 || !right3);
		}
		...
		public void SideshowSpelleater_AT_098()
		{
			...
			game.Process(PlayCardTask.Minion(game.CurrentPlayer, testCard));
			Assert.Equal(game.CurrentOpponent.Hero.Power.Card.Id, game.CurrentPlayer.Hero.Power.Card.Id);
		}
		...
		public void MasterJouster_AT_112()
		{
			...
			Assert.Equal(false, ((Minion)testCard1).HasDivineShield);
			game.Process(EndTurnTask.Any(game.CurrentPlayer));
			...
			Assert.Equal(true, ((Minion)testCard2).HasDivineShield);
		}
		...
		public void GrandCrusader_AT_118()
		{
			...
			game.Process(PlayCardTask.Spell(game.CurrentPlayer, testCard));
			Assert.Equal(5, game.CurrentPlayer.Hand.Count);
			Assert.Equal(ECardClass.PALADIN, game.CurrentPlayer.Hand[4].Card.Class);
		}
		...
		public void TheSkeletonKnight_AT_128()
		{
			...
			Assert.Equal(7, game.CurrentOpponent.Hand.Count);
		}
		...
		public void FjolaLightbane_AT_129()
		{
			...
			game.Process(PlayCardTask.SpellTarget(game.CurrentPlayer, spell, testCard));
			Assert.Equal(1, game.CurrentPlayer.Hand.Triggers.Count);
			Assert.Equal(true, ((Minion)testCard).HasDivineShield);
		}
		...
		public void JusticarTrueheart_AT_132()
		{
			...
			game.Process(PlayCardTask.Minion(game.CurrentPlayer, testCard));
			Assert.Equal(true, game.CurrentPlayer.Hero.Power.Card.Id.StartsWith("AT_132"));
		}
