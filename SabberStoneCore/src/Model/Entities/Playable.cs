...
namespace SabberStoneCore.Model.Entities
{
	...
	public interface IPlayable : ITargeting
	{
		...
		//void ActivateTask(PowerActivation activation, IPlayable target = null);
	...
	public abstract partial class Playable<T> : Targeting, IPlayable where T : Entity
	{
		...
		/// <autogeneratedoc />
		//public virtual void ApplyPowers(PowerActivation activation, Zone zoneType, IPlayable target = null)
		//{
		//	if (Powers == null || Powers.Count == 0)
		//		return;

		//	var removePowers = new List<Power>();
		...
		//	removePowers.ForEach(p => Powers.Remove(p));
		//}
		public void ActivateTask(PowerActivation activation, IPlayable target = null)
		{
			if (Powers == null) return;

			for (int i = 0; i < Powers.Count; i++)
			{
				//if (Powers[i].PowerTask == null) continue;
				ISimpleTask task = null;
				switch (activation)
				{
					case PowerActivation.POWER:
						task = Powers[i].PowerTask;
						break;
					case PowerActivation.DEATHRATTLE:
						task = Powers[i].DeathrattleTask;
						break;
					case PowerActivation.COMBO:
						task = Powers[i].ComboTask;
						break;
				}
